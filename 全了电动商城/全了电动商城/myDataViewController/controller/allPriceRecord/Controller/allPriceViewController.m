...
@interface allPriceViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation allPriceViewController
    ...
    [session GET:urlStr parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if (p>1) {
            ...
            for (NSDictionary *tempDic in dataArray) {
                ...
                allModel *goodsModel = [allModel setAllDataWithDic:tempDic];
...
-(void)headerRefreshBtns{
    p = 1;
    LDWeakSelf(self);
    NSString *urlStr = [NSString stringWithFormat:@"http://myadmin.all-360.com:8080/Admin/AppApi/gouMaiList/pagesize/%d/nowpage/%d/uid/%@",LoadingNumber, p , _userID];
