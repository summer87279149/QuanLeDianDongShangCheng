...
//为空的时候显示的视图
@property (nonatomic , strong)UIView *nilViews;
...
- (void)setNilView {
    _nilViews = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    _nilViews.backgroundColor = kColor_RGB(247, 247, 247);
    [self.view addSubview:_nilViews];
    _nilViews.hidden = YES;
    UILabel *label = [UILabel new];
    label.text = @"充值记录为空 、快去充值吧 !";
    label.textColor = [UIColor grayColor];
    label.textAlignment = NSTextAlignmentCenter;
    label.font = [UIFont systemFontOfSize:13];
    [_nilViews addSubview:label];
    UIImageView *imageView = [UIImageView new];
    imageView.image = [UIImage imageNamed:@"收藏"];
    [_nilViews addSubview:imageView];
    ...
    [session GET:urlStr parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if (p>1) {
            ...
            for (NSDictionary *tempDic in dataArray) {
                ...
                ChongZhiModel *goodsModel = [ChongZhiModel setIntergralWithDic:tempDic];
...
- (void)setRefreshTheUpAndDown {
    [self.tableView addHeaderWithTarget:self action:@selector(headerBtn)];
    [self.tableView addFooterWithTarget:self action:@selector(foorterBtn)];
    //队里延迟显示
...
-(void)headerBtn{
    p = 1;
    LDWeakSelf(self);
    NSString *urlStr = [NSString stringWithFormat:@"http://myadmin.all-360.com:8080/Admin/AppApi/amount/pagesize/%d/nowpage/%d/uid/%@/type/9",LoadingNumber, p , self.userID];
