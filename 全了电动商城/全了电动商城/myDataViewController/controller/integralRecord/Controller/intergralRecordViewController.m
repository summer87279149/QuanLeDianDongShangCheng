//
//  intergralRecordViewController.m
//  全了电动商城
//
...
#import "intergralRecordViewController.h"
#import "intergralRecordTableViewCell.h"
#import "intergralModel.h"

@interface intergralRecordViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation intergralRecordViewController
...
- (void)setTableViewController {
    ...
    [_tableView registerNib:[UINib nibWithNibName:@"intergralRecordTableViewCell" bundle:nil] forCellReuseIdentifier:reuseIndentifier];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    intergralRecordTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:reuseIndentifier forIndexPath:indexPath];
    intergralModel *model = self.dataModelArray[indexPath.row];
    cell.model = model;
    return cell;
}
    ...
    [session GET:urlStr parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if (p>1) {
            ...
            for (NSDictionary *tempDic in dataArray) {
                ...
                intergralModel *goodsModel = [intergralModel setIntergralDataWithDic:tempDic];
...
-(void)headerRefreshBtns{
    p = 1;
    LDWeakSelf(self);
    NSString *urlStr = [NSString stringWithFormat:@"http://myadmin.all-360.com:8080/Admin/AppApi/jifen/pagesize/%d/nowpage/%d/uid/%@",LoadingNumber, p , _userID];
