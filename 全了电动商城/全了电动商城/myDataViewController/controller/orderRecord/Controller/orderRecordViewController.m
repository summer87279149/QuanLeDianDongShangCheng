...
- (void)setTableViewController {
    ...
    [_tableView registerNib:[UINib nibWithNibName:@"orderRecordTableViewCell" bundle:nil] forCellReuseIdentifier:reuseIndentifier];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    orderRecordTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:reuseIndentifier forIndexPath:indexPath];
    orderReadModel *model = self.dataModelArray[indexPath.row];
    cell.model = model;
    return cell;
}
    ...
    [session GET:urlStr parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if (p>1) {
            ...
            for (NSDictionary *tempDic in dataArray) {
                ...
                orderReadModel *goodsModel = [orderReadModel setDataWithDic:tempDic];
...
-(void)headerRefreshBtns{
    p = 1;
    LDWeakSelf(self);
    NSString *urlStr = [NSString stringWithFormat:@"http://myadmin.all-360.com:8080/Admin/AppApi/saiDanShare/uid/%@/pagesize/%d/nowpage/%d",self.userID,LoadingNumber, p ];
