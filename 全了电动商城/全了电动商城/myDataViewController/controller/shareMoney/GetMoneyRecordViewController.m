...
- (void)viewDidLoad {
    ...
    [QLRequest getMoneyRecordSuccess:^(id response) {
        if ([response[@"code"]intValue]!=96100) {
            return ;
        }
        NSArray *arr = response[@"data"];
        for (NSDictionary*dic  in arr) {
            GetMoneyRecordModel*model = [[GetMoneyRecordModel alloc]initWithDictionary:dic];
