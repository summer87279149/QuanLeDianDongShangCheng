...
@interface WinningViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
- (void)setCollectionView {
    ...
    flowLayout.itemSize = CGSizeMake(SCREEN_WIDTH - 16, 180);
    flowLayout.sectionInset = UIEdgeInsetsMake(8, 8, 8, 8);
    _collectionView = [[UICollectionView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT) collectionViewLayout:flowLayout];
    _collectionView.backgroundColor = kColor_RGB(240, 237, 237);
    _collectionView.showsVerticalScrollIndicator = NO;
    _collectionView.showsHorizontalScrollIndicator = NO;
    _collectionView.delegate = self;
    _collectionView.dataSource = self;
    /** 注册collectionView cell 的重用池*/
    [_collectionView registerNib:[UINib nibWithNibName:@"WinningCollectionViewCell" bundle:nil] forCellWithReuseIdentifier:reuseIdentifier];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    WinningCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:reuseIdentifier forIndexPath:indexPath];
    cell.backgroundColor = [UIColor whiteColor];
    if (IS_IPHONE_6P) {
        cell.titleLine.constant = 30;
        cell.dataLine.constant  = 30;
        cell.labelLine.constant = 30;
        cell.imageLine.constant = 25;
        cell.headLine.constant  = 30;
        cell.shopImageLine.constant = 25;
    }
    winningModel *model = self.dataModelArray[indexPath.row];
    cell.model = model;
    return cell;
}
    ...
    [session GET:urlStr parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if (p>1) {
            ...
            for (NSDictionary *tempDic in dataArray) {
                ...
                winningModel *goodsModel = [winningModel setWinningDataWithDic:tempDic];
            ...
            //结束上拉加载动作
            [weakself.collectionView footerSetState:CoreFooterViewRefreshStateSuccessedResultDataShowing];
            //弹出小窗口 显示加载成功
            ...
            //结束下拉加载的动画
            [weakself.collectionView headerSetState:CoreHeaderViewRefreshStateSuccessedResultDataShowing];
        ...
        [weakself.collectionView headerSetState:CoreHeaderViewRefreshStateRefreshingFailed];
        [weakself.collectionView footerSetState:CoreFooterViewRefreshStateFailed];
        LDLog(@"失败 = %@",error);
    }];
}
//添加上下刷新的控件
- (void)setRefreshTheUpAndDown {
    [self.collectionView addHeaderWithTarget:self action:@selector(headerRefreshEvent)];
    [self.collectionView addFooterWithTarget:self action:@selector(foorterRefreshEvent)];
    //队里延迟显示
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        //数据刷新中
        [self.collectionView headerSetState:CoreHeaderViewRefreshStateRefreshing];
    });
...
-(void)headerRefreshEvent{
    p = 1;
    LDWeakSelf(self);
    NSString *urlStr = [NSString stringWithFormat:@"http://myadmin.all-360.com:8080/Admin/AppApi/saiDanShare/cpid/%@/pagesize/%d/nowpage/%d",self.cpid, LoadingNumber, p];
    ...
    if (weakself.dataModelArray.count == 0) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self.collectionView headerSetState:CoreHeaderViewRefreshStateSuccessedResultDataShowing];
        });
    }else{
        [weakself.collectionView headerSetState:CoreHeaderViewRefreshStateRefreshingFailed];
    }
